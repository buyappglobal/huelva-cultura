...
export default function SmartTVPlayer() {
  ...
  const isPublicMode = slug === 'public' || searchParams.get('public') === 'true';
  
  const [clientId, setClientId] = useState<string | null>(
    isPublicMode ? 'public' : (isDemoMode ? 'demo' : (slug || searchParams.get('id') || localStorage.getItem('aura_tv_client_id')))
  );
  ...
  useEffect(() => {
    ...
    // No conectar SSE para modo public (no necesitamos sockets para reproductor público)
  ...
  return (
    <div 
      ...
      onClick={() => {
        if (!isPlaying) {
          if (audioRef.current) audioRef.current.play().catch(e => console.warn("Audio auto-play failed", e));
          setIsPlaying(true);
    ...
    >
      ...
      {!isShowingAd && (
        <AuraBackgroundPlayer
          ...
          isPlaying={isPlaying || true} // Ensure animations run even if audio is paused/blocked
      ...
      {!isShowingAd && (() => {
        ...
        if (!isPlaying) {
          ...
          if (manifest?.visuals?.quotes && Array.isArray(manifest.visuals.quotes) && manifest.visuals.quotes.length > 0) {
            const arr = manifest.visuals.quotes;
            quoteText = arr[localQuoteIndex % arr.length];
          }
        ...
        return (
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 50, pointerEvents: 'none' }}>
            <div style={{ background: 'radial-gradient(ellipse at center, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0) 70%)', padding: '10vw 15vw', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <AnimatePresence mode="wait">
                ...
                >
                  <h2 style={{ fontFamily: '"AuraElegantSerif", Georgia, serif', fontSize: `${2.8 * scale}vw`, color: 'white', letterSpacing: '0.05em', fontWeight: 'normal', fontStyle: 'italic', margin: 0, textTransform: 'none', textShadow: '0 4px 30px rgba(0,0,0,1), 0 0 50px rgba(0,0,0,0.8), 0 0 10px rgba(0,0,0,1)', textAlign: 'center', lineHeight: '1.4', contain: 'layout text' }}>
                    {quoteText}
                  </h2>
                  <p style={{ fontFamily: '"Inter", "Helvetica Neue", Helvetica, Arial, sans-serif', color: 'rgba(255,255,255,0.85)', marginTop: '24px', fontSize: `${1.1 * scale}vw`, letterSpacing: '6px', textTransform: 'uppercase', fontWeight: '500', textShadow: '0 4px 30px rgba(0,0,0,1), 0 0 50px rgba(0,0,0,0.8), 0 0 10px rgba(0,0,0,1)', textAlign: 'center', contain: 'layout text' }}>
                    {quoteCategory}
                  </p>
                </motion.div>
              </AnimatePresence>

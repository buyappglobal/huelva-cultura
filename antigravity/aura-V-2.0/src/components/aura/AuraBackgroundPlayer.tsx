...
export const AuraBackgroundPlayer: React.FC<AuraBackgroundPlayerProps> = ({
  ...
  const renderFallbackTheme = () => {
    ...
    const baseIndex = fallbackThemeIndex % 14;
    ...
    switch (baseIndex) {
      ...
      case 10: {
        // Theme 10: Cyan Particle Bloom (Radial Dot Cluster)
        const v = variationSeed;
        const rings = 4 + (v % 3);
        ...
        for (let r = 1; r <= rings; r++) {
          const radius = r * (30 + v * 5);
          for (let i = 0; i < dotsPerRing; i++) {
            const angle = (i * (360 / dotsPerRing)) + (r * 15);
            const delay = (r * 0.2) + (i * 0.05);
            elements.push(
              <div key={`p10-${r}-${i}`} style={{
                position: 'absolute',
                ...
                left: '50%', top: '50%',
                transform: `rotate(${angle}deg) translateY(-calc(${radius}px * (1 + var(--audio-subbass, 0) * 0.8)))`,
      ...
      case 11: {
        // Theme 11: Golden Fibonacci Spiral
        const v = variationSeed;
        const totalDots = 60 + v * 10;
        const color = 'rgba(255, 215, 0, 0.9)';
        const elements = [];
        const phi = (1 + Math.sqrt(5)) / 2;
        
        for (let i = 0; i < totalDots; i++) {
          const radius = Math.pow(i, 0.6) * 15;
          ...
          elements.push(
            <div key={`p11-${i}`} style={{
              ...
              left: '50%', top: '50%',
              transform: `rotate(${angle}deg) translateX(calc(${radius}px * (1 + var(--audio-mid, 0) * 0.5)))`,
              animation: `pulseGlow 1.5s infinite ease-in-out alternate`,
              animationDelay: `${delay}s`,
              willChange: 'transform'
            }} />
          );
        ...
        return (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', pointerEvents: 'none' }}>
            <div style={{ position: 'relative', width: 0, height: 0, animation: 'spin 60s linear infinite' }}>
              {elements}
            </div>
      ...
      case 12: {
        // Theme 12: Esfera 3D Pulsante (Fibonacci Sphere) + Dual Spheres Variation
        const v = variationSeed;
        const isDual = v % 2 === 1; // Alternates between single and dual based on v
        const N_outer = 200 + (v * 15);
        const N_inner = Math.floor(N_outer * 0.5);
        const outerElements = [];
        const innerElements = [];
        const R = 180 + (v * 5);
        ...
        for (let i = 0; i < N_outer; i++) {
          const theta = Math.acos(1 - 2 * (i + 0.5) / N_outer);
          ...
          outerElements.push(
            <div key={`outer-${i}`} style={{
              ...
              borderRadius: '50%',
              background: '#fff',
              boxShadow: isDual ? '0 0 10px #0ff, 0 0 20px #0ff' : '0 0 10px #f0f, 0 0 20px #f0f',
              left: '50%', top: '50%',
              transform: `translate3d(${x}px, ${y}px, ${z}px) scale(calc(1 + var(--audio-treble, 0) * 2.5))`,
              opacity: 0.7
            }} />
          );
        }
        ...
        // Inner Sphere
        if (isDual) {
          const R_inner = R * 0.45;
          for (let i = 0; i < N_inner; i++) {
            const theta = Math.acos(1 - 2 * (i + 0.5) / N_inner);
            const phi = Math.PI * (1 + Math.sqrt(5)) * i;
            
            const x = R_inner * Math.sin(theta) * Math.cos(phi);
            const y = R_inner * Math.sin(theta) * Math.sin(phi);
            const z = R_inner * Math.cos(theta);
            ...
            innerElements.push(
              <div key={`inner-${i}`} style={{
                ...
                borderRadius: '50%',
                background: '#fff',
                boxShadow: '0 0 15px #f0f, 0 0 30px #f0f',
                left: '50%', top: '50%',
                transform: `translate3d(${x}px, ${y}px, ${z}px) scale(calc(1 + var(--audio-mid, 0) * 2))`,
                opacity: 0.9
              }} />
            );
          }
        ...
        return (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', pointerEvents: 'none', perspective: '800px' }}>
            <div style={{ position: 'relative', width: 0, height: 0, transform: 'scale(calc(1 + var(--audio-bass, 0) * 0.4))', transformStyle: 'preserve-3d' }}>
              
              <div style={{
                position: 'absolute', left: 0, top: 0, width: 0, height: 0,
                transformStyle: 'preserve-3d',
                animation: 'spinOuter 40s linear infinite'
              }}>
                {outerElements}
              </div>
      ...
      case 13: {
        // Theme 13: ADN Doble Hélice (DNA)
        const v = variationSeed;
        const N = 60 + (v * 10);
        const L_helix = 400 + (v * 20); // total height
        const elements = [];
        const R = 50 + (v * 5); // radius of helix

        for (let i = 0; i < N; i++) {
          const theta = (i / N) * 4 * Math.PI;
          const y = (i / N) * L_helix - (L_helix / 2);
          
          const x1 = R * Math.cos(theta);
          const z1 = R * Math.sin(theta);
          
          const x2 = R * Math.cos(theta + Math.PI);
          const z2 = R * Math.sin(theta + Math.PI);
          
          // Rungs (links)
          elements.push(
            <div key={`rung-${i}`} style={{
              position: 'absolute',
              width: `${R * 2}px`, height: '2px',
              background: 'rgba(255, 255, 255, 0.15)',
              left: '50%', top: '50%',
              marginLeft: `-${R}px`,
              transform: `translate3d(0, ${y}px, 0) rotateY(${-theta}rad) scaleX(calc(1 + var(--audio-treble, 0) * 0.5))`,
              transformOrigin: 'center'
            }} />
          );
          ...
          // Strand 1
          elements.push(
            <div key={`dna1-${i}`} style={{
              position: 'absolute',
              width: '6px', height: '6px', borderRadius: '50%',
              background: '#f0f', boxShadow: '0 0 10px #f0f',
              left: '50%', top: '50%',
              marginLeft: '-3px', marginTop: '-3px',
              transform: `translate3d(${x1}px, ${y}px, ${z1}px) scale(calc(1 + var(--audio-bass, 0) * 1.5))`
            }} />
          );

          // Strand 2
          elements.push(
            <div key={`dna2-${i}`} style={{
              position: 'absolute',
              width: '6px', height: '6px', borderRadius: '50%',
              background: '#0ff', boxShadow: '0 0 10px #0ff',
              left: '50%', top: '50%',
              marginLeft: '-3px', marginTop: '-3px',
              transform: `translate3d(${x2}px, ${y}px, ${z2}px) scale(calc(1 + var(--audio-bass, 0) * 1.5))`
            }} />
          );
        }

        return (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', pointerEvents: 'none', perspective: '1000px' }}>
            <div style={{ transform: 'translateY(calc(var(--audio-subbass, 0) * 20px))', transformStyle: 'preserve-3d' }}>
              <div style={{
                position: 'relative', width: 0, height: 0,
                transformStyle: 'preserve-3d',
                animation: 'spinDNA 15s linear infinite'
              }}>
                {elements}
              </div>
            </div>

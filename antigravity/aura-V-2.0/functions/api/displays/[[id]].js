  ...
  if (request.method === "POST") {
    try {
      ...
      if (!current) {
        ...
        const city = user ? user.city : "Madrid, ES";
               await env.DB.prepare(
          `INSERT INTO displays (id, establishmentName, adminTitle, location, theme, volume, isZenMode, isNoDistractionsMode, isRemoteControl, performanceMode, signageUrl, signageType, signageExpiresAt, compiledManifest, visualStyle, vjConfig, textSize, promoFlashText, promoFlashExpiresAt, updatedAt)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ).bind(
          resolvedId,
        ...
        await env.DB.prepare(
          `UPDATE displays SET 
             ...
             promoFlashText = ?, promoFlashExpiresAt = ?, updatedAt = ?
           WHERE id = ?`
        ).bind(

  ...
  if (request.method === "POST") {
    try {
      ...
      if (current) {
        ...
        if (finalPartnerId && finalPartnerId !== current.partnerId) {
          try {
            await sendNotification(
              env,
              finalPartnerId,
              "Nuevo Cliente Asignado 📂",
              `Se te ha asignado el cliente ${merged.email} (${merged.city || "Sin ciudad"}).`,
              "assigned_partner",
              "/"
            );
          } catch (notifErr) {
            console.error("Error sending assigned partner notification", notifErr);
          }
        }
